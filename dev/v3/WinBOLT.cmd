...
REM ###################################################################################################################
SET LAST_UPDATED=02.15.2016
SET Current_Version=B50
SET wsize=10096
REM ###################################################################################################################
...
REM **v3.3: added system restore
REM **v3.3: added Kaspersky Virus Remover
REM **v3.3: added AdwCleaner
REM **v3.3: added ticker count
REM **v3.3: added Spy Bot Search & Destroy
REM **v3.3: updated EEKv9 to EEKv11 (duo engine scanner)
REM **v3.3: added Windows Edition Dection
REM ###################################################################################################################
REM ###################################################################################################################
...
:copy
IF EXIST C:\WinBOLT\Backups\Logs\EEK_Qurantine GOTO fileman
1>nul 2>nul md C:\WinBOLT\
1>nul 2>nul md C:\WinBOLT\Backups
1>nul 2>nul md C:\WinBOLT\Backups\Logs
1>nul 2>nul md C:\WinBOLT\Backups\Logs\AdwCleaner
1>nul 2>nul md C:\WinBOLT\Backups\Logs\EEK_Quarantine
@powershell remove-item C:\WinBOLT\EEK* -Force -Recurse -Erroraction 'silentlycontinue'
...
1>nul 2>nul del /f C:\WinBOLT\WinBOLT.bat
...
if errorlevel 0 (goto power) else (
  goto 7W
)
...
REM Older systems that are NOT supported
:old
color CF
cls
echo.
echo.
echo Your system is running Windows XP or older.
echo WinBOLT will most likely run but might not work correctly.
echo.
echo.
...
echo           [X] This product is licensed under GNU General Public v2.
echo.
echo      System Requirements:
echo           [X] .NET Framework version 3.5 and above is required.
echo           [X] PowerShell version 3.0 and above is required.
echo.
echo.
echo.
echo.
...
echo.
echo         ###########################################################################
@powershell Write-Host "`t" -f white -nonewline; write-host "`# WinBOLT v%Current_Version% -" -f white -nonewline; write-host "Maintenance Automation Utility" -f magenta -nonewline; write-host "- SourceForge.net/p/winbolt `#" -f white
echo         ###########################################################################
@powershell Write-Host "`t`t`t`t`t`t`tLast Updated: `(" -f white -nonewline; Write-Host "%Last_Updated%" -f red -nonewline; write-host "`)" -f white
echo.
@powershell Write-Host "`t`tv%Current_Version% IS A BETA RELEASE - PLEASE REPORT ALL BUGS" -f red
echo.
echo.
@powershell write-host "`t1." -f green -nonewline; write-host "Install Chocolatey" -f white
@powershell write-host "`t2." -f green -nonewline; write-host "Run Windows Update and Chocolatey Updates" -f white
@powershell write-host "`t3." -f green -nonewline; write-host "Enable Maintenance Script 'Runs Monthly Each 16th @ 10PM'" -f white
@powershell write-host "`t4." -f green -nonewline; write-host "Rkill`, TDSSKiller`, CCleaner`, AdwCleaner`, SpyBot`, KVR`, EEK `& CryptoPrevent." -f white
@powershell write-host "`t5." -f green -nonewline; write-host "Defrag HDD`, Sys File CHK`, File Sys CHK `(Auto reboot once completed`)" -f white
@powershell write-host "`t6." -f green -nonewline; write-host "*All Of The Above - Full Blown System Maintenance" -f white
echo.
echo.
echo         Reboot now supported, just add "r" to your selection.
@powershell write-host "`tExample:" -f white -nonewline; write-host "`t1r" -f green -nonewline; write-host "`tor" -f white -nonewline; write-host "`t2r" -f green -nonewline; write-host "`tor" -f white -nonewline; write-host "`t24r" -f green
echo.
echo.
@powershell write-host "`tN." -f green -nonewline; write-host "Second Menu" -f white
@powershell write-host "`tX." -f green -nonewline; write-host "Exit" -f red
echo.
echo.
echo.
...
echo    # WinBOLT v%Current_Version% - Maintenance Automation Tool - SourceForge.net/p/winbolt #
echo    ##########################################################################
echo      Special Tools ^& Actions Menu         (Last Updated: %Last_Updated%)
echo.
...
echo         19)  Full System Scan - Emsisoft v11
echo         20)  CryptoPrevent - Apply System Protection
echo         21)  Create System Restore Point
echo         22)  Full System Scan - AdwCleaner
echo.
echo    100) [Thunder Scan] Option: 14, 15, 16, 17, 18, 19, 20, 21, 22.
echo.
echo.
echo     i)  Information on WinBOLT
...
echo         22)  Full System Scan - AdwCleaner
echo.
echo    100) [Limited Thunder Scan] Option: 14, 15, 18, 19, 20, 21, 22.
echo.
echo.
echo     i)  Information on WinBOLT
echo     R)  Return to the second menu
...
if %OS%==32BIT pushd "C:\WinBOLT\EEK\bin32\"
if %OS%==64BIT pushd "C:\WinBOLT\EEK\bin32\"
1>nul 2>nul a2cmd.exe /update
1>nul 2>nul "a2cmd.exe" /f="C:\" /rk /m /t /pup /n /l="C:\WinBOLT\EEK\Logs\Live_Emsisoft_Scan.log" /q="C:\WinBOLT\Backups\Logs\EEK_Qurantine\"
For /f "tokens=2-4 delims=/ " %%a in ('date /t') do (set mdate=%%a.%%b.%%c)
For /f "tokens=1-2 delims=/:" %%a in ('time /t') do (set mtime=%%a.%%b)
...
For /f "tokens=1-2 delims=/:" %%a in ('time /t') do (set mtime=%%a.%%b)
1>nul 2>nul copy "C:\WinBOLT\EEK\Logs\Live_Emsisoft_Scan.log" "C:\WinBOLT\Backups\Logs\%mdate%_%mtime%_Emsisoft_Scan.log"
@powershell write-host "-FINISHED!" -foreground "magenta"
...
echo.
echo    Important downloads are taking place; please wait.
REM Installing AutoIT Language in the background
:: AITI.bat
IF EXIST C:\programdata\chocolatey\bin\AutoIt3.exe (
    goto 4C
) else (
    1>nul 2>nul choco install autoit.commandline -y --force
)
...
REM ENABLES SYSTEM RESTORE POINTS
@powershell "Enable-ComputerRestore -Drive "%systemdrive%" | Out-Null"

REM - CREATES RESTORE POINT WITH TIMESTAMP
@powershell "Checkpoint-Computer -Description 'WinBOLTv%Current_Version%_CourtesyCheckPoint' -RestorePointType MODIFY_SETTINGS | Out-File -filepath "C:\WinBOLT\Backups\Logs\SysRestore_Completed_%mdate%.log"
@powershell write-host "-FINISHED!" -foreground "magenta"
1>nul 2>nul timeout /t 20
...
echo.
echo    Running AdwCleaner, please wait.
@powershell remove-item C:\AdwCleaner* -Force -Recurse

pushd "C:\WinBOLT\repo\"
1>nul 2>nul cscript.exe AIT.vbs
1>nul 2>nul cscript.exe ACA.vbs
:MON
timeout /t 1 >nul
tasklist /fi "windowtitle eq - AdwCleaner - Information -" | find /i "adwcleaner.exe" > nul
if not errorlevel 1 (goto 4EEK) else (
  goto MON
)
:4EEK
@powershell remove-item C:\AdwCleaner* -Force -Recurse
...
echo.
@powershell write-host "Completed your request. You will redirect to the menu." -foreground "cyan"
@powershell write-host "Please check C:\WinBOLT\Backups\Logs for more information." -foreground "cyan"
timeout /t 30 >nul
goto 1menu
...
echo.
echo    ################################################################
echo    -Creating a System Restore Point, cancel anytime with (Ctr + C)-
echo    ################################################################
echo.
echo.
echo.
...
echo.
echo    ###################################################
echo    -Running AdwCleaner, cancel anytime with (Ctr + C)-
echo    ###################################################
echo.
echo.
echo.
...
timeout /t 1 >nul
tasklist /fi "windowtitle eq - AdwCleaner - Information -" | find /i "adwcleaner.exe" > nul
if not errorlevel 1 (goto 99EEK) else (
  goto 808
)
:99EEK
@powershell remove-item C:\AdwCleaner* -Force -Recurse
...
echo.
echo    ###############################################################
echo    -( Thunder Scan Completed - RKill, AdwCleaner, CryptoPrevent, Sophos, Vipre, Emsisoft and MBAM )-
echo    ###############################################################
echo.
echo.
...
echo.
echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
echo Thanks To: @Roman @Devon @GilPerez @Aura @BigDawgBilly
echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
echo.
